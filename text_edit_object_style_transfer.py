...
def finetune_style(opt, model_path, iteration, views, gaussians, pipeline, background, classifier, OBJ_ID, cameras_extent, removal_thresh, epoch, scale_reg_loss):
    ...
    clip_preprocess = T.Compose([
        T.Resize(224),                  # Resize the shorter side to 224 pixels
        T.CenterCrop(224),              # Center-crop the image to 224x224
...
def style(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool, opt : OptimizationParams, select_obj_id : list, removal_thresh : float,  epoch: int, scale_reg_loss: float):
    ...
    # 2. style selected object
        gaussians, pcd_path = finetune_style(opt, dataset.model_path, scene.loaded_iter, scene.getTrainCameras(), gaussians, pipeline, background, classifier, obj_id, scene.cameras_extent, removal_thresh, epoch, scale_reg_loss)
...
if __name__ == "__main__":
    ...
    style(model.extract(args), args.iteration, pipeline.extract(args), args.skip_train, args.skip_test, opt.extract(args), args.select_obj_id, args.removal_thresh, args.epoch, args.scale_reg_loss)